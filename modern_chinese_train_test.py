...
def load_data(path):
    ...
    with open(path, 'r', encoding='utf-8') as f:
        ...
        train_former = [line[0] for line in lines[:32472]]
        train_quote = [line[1] for line in lines[:32472]]
        train_latter = [line[2] for line in lines[:32472]]
        valid_former = [line[0] for line in lines[32472:36657]]
        valid_quote = [line[1] for line in lines[32472:36657]]
        valid_latter = [line[2] for line in lines[32472:36657]]
        test_former = [line[0] for line in lines[36657:]]
        test_quote = [line[1] for line in lines[36657:]]
        test_latter = [line[2] for line in lines[36657:]]
...
data_path = "./data/modern_chinese.txt"
...
def make_quote_tensors(quote):
    ...
    for q in quotes:
        encoded_dict = tokenizer.encode_plus(q,
                                             ...
                                             return_tensors='pt')
        input_ids.append(encoded_dict['input_ids'])
    input_ids = torch.cat(input_ids, 0)  # [num, 100]
    ...
    for epoch in range(epoch):
        ...
        with torch.no_grad():
            ...
            if total_acc > best_acc:
                best_acc = total_acc
                torch.save(
                    model.quote_model.state_dict(), "./model/modern_chinese_quote.pth")
                torch.save(
                    model.contex_model.state_dict(), "./model/modern_chinese_context.pth")
...
save_model_state = torch.load("./model/modern_chinese_quote.pth")
...
def training_mask(model, epoch, train, valid, quote_tensor, device):
    ...
    for epoch in range(epoch):
        ...
        if total_MRR > best_MRR:
            best_MRR = total_MRR
            torch.save(model, "./model/model_modern_chinese.model")
...
test_loader = DataLoader(dataset=test_dataset,
                         ...
                         num_workers=2)
print('loading model ...')
model = torch.load('./model/model_modern_chinese.model')
model.to(device)
test(model=model,
